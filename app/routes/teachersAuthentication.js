module.exports = function (app, passport) {
    ...
    var Teachers = require('../models/tachers');
    ...
    app.get('/email-confirmation/:emailToken', function (req, res) {
      ...
      asynq.waterfall([
        function (done) {
            Teachers.findOne({ 'emailConfirmationToken': token },
            function (err, user) {
              if (!user) {
    ...
    app.post('/password-recovery', function (req, res, next) {
      asynq.waterfall([
        ...
        function (token, done) {
            Teachers.findOne({ 'email': req.body.email }, function (err, user) {
    ...
    app.get('/password-reset/:token', function (req, res) {
        Teachers.findOne({ 'resetPasswordToken': req.params.token, 'resetPasswordExpires': { $gt: Date.now() } },
        function (err, user) {
          if (!user) {
            req.flash('passwordRecoveryMessage', 'No user found with that email.');
            return res.redirect('/password-recovery');
